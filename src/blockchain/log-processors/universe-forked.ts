...
import { updateMarketState, updateMarketStatePromise } from "./database";
...
export async function processUniverseForkedLog(db: Knex, augur: Augur, log: FormattedEventLog) {
  ...
  db("universes").update("forked", true).where({ universe: log.universe });
...
export async function processUniverseForkedLogRemoval(db: Knex, augur: Augur, log: FormattedEventLog) {
  ...
  db("markets").decrement("needsDisavowal", 1).where({ universe: log.universe }).whereNot("marketId", forkingMarket.marketId);
  db("universes").update("forked", false).where({ universe: log.universe });
}
