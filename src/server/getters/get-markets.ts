...
import { getMarketsWithReportingState, queryModifier, marketPassesInitialREPFilter } from "./database";
...
export async function getMarkets(db: Knex, augur: {}, params: t.TypeOf<typeof GetMarketsParams>): Promise<Array<Address>> {
  ...
  if (params.enableInitialRepFilter) {
    ...
    marketsRows = _.filter(marketsRows, (row) => {
      ...
      return marketPassesInitialREPFilter(totalInitialREPStakeByMarket[row.marketId].endTime, totalInitialREPStakeByMarket[row.marketId].totalInitialREPStake);
    });
  }
