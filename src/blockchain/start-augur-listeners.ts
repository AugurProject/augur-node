...
export function startAugurListeners(db: Knex, augur: Augur, highestBlockNumber: number, databaseDir: string, errorCallback: ErrorCallback): BlockAndLogsQueue {
  const blockAndLogsQueue = new BlockAndLogsQueue(async (direction: BlockDirection, block: BlockDetail, logs: Array<FormattedEventLog>) => {
    return processBlockAndLogs(db, augur, direction, block, false, logs, databaseDir).catch((err) => {
      errorCallback(err);
      throw(err);
    });
  });
