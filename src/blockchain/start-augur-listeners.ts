...
export function startAugurListeners(db: Knex, pouch: PouchDB.Database, augur: Augur, highestBlockNumber: number, errorCallback: ErrorCallback): BlockAndLogsQueue {
  const blockAndLogsQueue = new BlockAndLogsQueue(async (direction: BlockDirection, block: BlockDetail, logs: Array<FormattedEventLog>) => {
    return processBlockAndLogs(db, pouch, augur, direction, block, false, logs).catch((err) => {
      errorCallback(err);
      throw(err);
    });
  });
